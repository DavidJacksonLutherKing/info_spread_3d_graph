//Define Namespace as tc for transmission chain application object.
var merkleTC = new Object();
merkleTC.temp = merkleTC.temp || {
    removedNodes: {},
    removedLinks: {},
    nodes: [],
    links: []
};

merkleTC.history = merkleTC.history || {
    rootData: new Object(),
    historyRecord: new Array(),
    maxHistoryNodeNum: 10,
};
merkleTC.svg = merkleTC.svg || {
    height: window.innerHeight,
    width: window.innerWidth,
    maxNodeSize: 500,
    x_browser: 20,
    y_browser: 25,
    root: undefined,
    links: undefined,
    nodes: undefined,
    nodeRadius: 20,
    rootRadius: 30,
    lineDistance: 80,
    lineStroke: 3,
    pathRadius: 104,
    showNodeTreeButtonRadius: 15,
    showNodeTreeButtonBorderWidth: 3,
    circleBorderWidth: 5,
    rootCircleBorderWidth: 7,
    fontSize: 12,
    manybodyStrength: -1000,
    personIconBase64: "data:image/png;base64,",
    showNodeTreeButtonImg: "",
    drag: function (simulation) {
        function dragstarted(d) {
            if (!d3.event.active) simulation.alphaTarget(0.3).restart();
            d.fx = d.x;
            d.fy = d.y;
        }

        function dragged(d) {
            d.fx = d3.event.x;
            d.fy = d3.event.y;
        }

        function dragended(d) {
            if (!d3.event.active) simulation.alphaTarget(0);
            d.fx = null;
            d.fy = null;
        }

        return d3.drag()
            .on("start", dragstarted)
            .on("drag", dragged)
            .on("end", dragended);
    }
}

//Define updateSvg Function
merkleTC.updateSvg = function (data) {
    //Define SVG Size
    merkleTC.svg.root = d3.hierarchy(data);
    merkleTC.svg.links = merkleTC.svg.root.links();
    merkleTC.svg.nodes = merkleTC.svg.root.descendants();
    merkleTC.svg.simulation = d3.forceSimulation(merkleTC.svg.nodes)
        .force("link", d3.forceLink(merkleTC.svg.links).id(d => d.data.customerID).distance(merkleTC.svg.lineDistance).strength(1))
        .force("charge", d3.forceManyBody().strength(merkleTC.svg.manybodyStrength))
        .force("x", d3.forceX())
        .force("y", d3.forceY());

    merkleTC.svg.svgElement = d3.create("svg")
        .attr("viewBox", [-merkleTC.svg.width / 2, -merkleTC.svg.height / 2, merkleTC.svg.width, merkleTC.svg.height])
        .call(d3.zoom()
            .scaleExtent([0.1, 5])
            .on("zoom", function () {
                merkleTC.svg.nodeElements.attr("transform", d3.event.transform);
                merkleTC.svg.lineElements.attr("transform", d3.event.transform);
                merkleTC.svg.nameElements.attr("transform", d3.event.transform);
                // merkleTC.svg.actionBoxElements.attr("transform", d3.event.transform);
                merkleTC.svg.childCountElements.attr("transform", d3.event.transform);
                // merkleTC.svg.checkCurrentNodeTreeButtonElements.attr("transform", d3.event.transform);
            }))
        .on('dblclick.zoom', null);


    merkleTC.svg.lineElements = merkleTC.svg.svgElement.append("g")
        .attr("class", "line")
        .selectAll("line")
        .data(merkleTC.svg.links)
        .join("line")
        .attr("id", d => d.source.data.customerID + "-" + d.target.data.customerID + "-line")
        .attr("name", d => d.target.data.customerID + "-line")
        .attr("class", "link-line")
        .attr("end", d => d.target.data.customerID)
        .attr("is_shown", "true")
        .attr("stroke", "#333")
        .attr("stroke-width", merkleTC.svg.lineStroke)
        .attr("stroke-opacity", 0.6);

    merkleTC.svg.nodeElements = merkleTC.svg.svgElement.append("g")
        .attr("class", "nodes")
        .selectAll("circle")
        .data(merkleTC.svg.nodes)
        .join("circle")
        .attr("class", "node-circle")
        .attr("is_shown", "true")
        .attr("children_shown", "true")
        .attr("r", d => (d.parent != null ? merkleTC.svg.nodeRadius : merkleTC.svg.rootRadius))
        .attr("fill", d => "url(#" + d.data.customerID + "-img)")
        .attr("id", d => d.data.customerID + "-circle")
        .attr("stroke", d => (d.parent != null ? "yellow" : "black"))
        .attr("stroke-width", d => (d.parent != null ? merkleTC.svg.circleBorderWidth : merkleTC.svg.rootCircleBorderWidth))
        .call(merkleTC.svg.drag(merkleTC.svg.simulation));

    merkleTC.svg.imgElements = merkleTC.svg.svgElement.append("defs")
        .selectAll("pattern")
        .data(merkleTC.svg.nodes)
        .join("pattern")
        .attr("id", d => d.data.customerID + "-img")
        .attr("width", "100%")
        .attr("height", "100%")
        .attr("patternContentUnits", "objectBoundingBox")
        .append("image")
        .attr("width", 1)
        .attr("height", 1)
        .attr("xlink:href", d => d.data.img);

    merkleTC.svg.nameElements = merkleTC.svg.svgElement.append("g")
        .attr("class", "nickname-box")
        .attr("is_shown", "true")
        .selectAll("text")
        .data(merkleTC.svg.nodes)
        .join("text")
        .attr("font-size", merkleTC.svg.fontSize)
        .attr("stroke", "black")
        .attr("width", d => d.data.nickName.length * merkleTC.svg.fontSize)
        .attr("height", 20)
        .text("")
        .attr('encryptedNickname', d => d.data.nickName)
        .attr('gender', d => d.data.gender)
        .attr("id", d => d.data.customerID + "-nickname")
        .attr("class", "nickname");

    merkleTC.svg.childCountElements = merkleTC.svg.svgElement.append("g")
        .attr("class", "childCount")
        .selectAll("text")
        .data(merkleTC.svg.nodes)
        .join("text")
        .attr("font-size", merkleTC.svg.fontSize)
        .attr("stroke", "black")
        .text(d => d.data.childCount)
        .attr("id", d => d.data.customerID + "-childCount")
    merkleTC.svg.simulation.on("tick", () => {
        merkleTC.svg.lineElements
            // .attr("d", d => "M" + d.source.x + "," + d.source.y + " A" + merkleTC.svg.pathRadius + "," + merkleTC.svg.pathRadius + ",0,0,1," + d.target.x + "," + d.target.y)
            .attr("x1", d => d.source.x)
            .attr("y1", d => d.source.y)
            .attr("x2", d => d.target.x)
            .attr("y2", d => d.target.y)
            .attr("stroke", "black")

        merkleTC.svg.nameElements
            .attr("x", d => (d.parent != null ? d.x - merkleTC.svg.nodeRadius : d.x - merkleTC.svg.rootRadius ))
            .attr("y", d => (d.parent != null ? d.y + merkleTC.svg.nodeRadius + merkleTC.svg.fontSize * 1.5 : d.y + merkleTC.svg.rootRadius + merkleTC.svg.fontSize * 1.5 ));

        merkleTC.svg.childCountElements
            .attr("x", d => (d.parent != null ? d.x + merkleTC.svg.nodeRadius : d.x + merkleTC.svg.rootRadius))
            .attr("y", d => (d.parent != null ? d.y + merkleTC.svg.nodeRadius : d.y + merkleTC.svg.rootRadius));

        merkleTC.svg.nodeElements
            .attr("cx", d => d.x)
            .attr("cy", d => d.y);
    });

    // console.log(merkleTC.svg.svgElement);
    return merkleTC.svg;
}


//Fetch Chain Data from JSON File and callback function to influence the data to d3 chart

d3.json("data/curve-tree-3.json").then(function(data ,err ){
    var div_id = "transmission-chain-chart";
    merkleTC.initialTransmissionChainChart(div_id,data);
})

merkleTC.initialTransmissionChainChart = function(div_id, data) {
    merkleTC.applicationDiv = {
        id:div_id,
    };
    merkleTC.svg.treeData = data;
    merkleTC.svg.linkData = {
        nodes: merkleTC.nodesFromTree(data),
        links: merkleTC.treeToLink(data)
    };
    merkleTC.history.historyRecord = [merkleTC.svg.treeData];
    merkleTC.renderChart(merkleTC.svg.treeData , div_id);
}

merkleTC.renderChart = function (data, div_id) {

    var topDiv = document.createElement("div");
    topDiv.setAttribute("id", "top-div");
    //Reset Button
    document.getElementById(div_id).innerHTML = "";
    var resetButtonDiv = document.createElement("div");
    resetButtonDiv.setAttribute("id", "reset-div");
    var resetButton = document.createElement("button");
    resetButton.setAttribute("id", "chart-reset-btn");
    resetButton.innerText = "重置";
    resetButtonDiv.appendChild(resetButton);
    var chartSVG = merkleTC.updateSvg(data);
    topDiv.append(resetButtonDiv);
    // resetButtonDiv.style.width = window.innerWidth + "px";

    //History Bar
    var historyBar = document.createElement("div");
    historyBar.setAttribute("id", "history-bar-div");
    var historyRec = Object.assign([], merkleTC.history.historyRecord);
    merkleTC.generateNewHistoryBar(historyBar, historyRec);
    topDiv.append(historyBar);
    document.getElementById(div_id).append(topDiv);

    //svg
    var svgDiv = document.createElement("div");
    svgDiv.setAttribute("id", "chain-svg-div");
    document.getElementById(div_id).append(svgDiv);
    document.getElementById("chain-svg-div").innerHTML = "";
    document.getElementById("chain-svg-div").append(chartSVG.svgElement.node());

    //action box

    var focusNodeDiv = document.createElement("div");
    focusNodeDiv.setAttribute("id", "current-node-div");

    var focusNodeLabel = document.createElement("div");
    focusNodeLabel.setAttribute("id", "current-node-label");
    focusNodeLabel.innerText = "当前节点:";
    var focusNodeNickname = document.createElement("div");
    focusNodeNickname.setAttribute("id", "current-node-nickname");
    focusNodeNickname.innerText = "";
    focusNodeDiv.append(focusNodeLabel);
    focusNodeDiv.append(focusNodeNickname);

    var actionButtonGroupDiv = document.createElement("div");
    actionButtonGroupDiv.setAttribute("id", "action-button-group-div");
    var showFocusNodeTreeButton = document.createElement("a");
    showFocusNodeTreeButton.innerText = '展示当前节点的传播链路图';
    showFocusNodeTreeButton.setAttribute("id", "show-focus-node-tree-button");
    showFocusNodeTreeButton.setAttribute("focus-node-id", "");
    showFocusNodeTreeButton.addEventListener("click", function () {
        var customerId = this.getAttribute("focus-node-id");
        var data = JSON.parse(this.getAttribute("data"));

        if (merkleTC.history.historyRecord[merkleTC.history.historyRecord.length - 1].customerID != data.customerID) {
            merkleTC.history.historyRecord.push(data);
            if (merkleTC.history.historyRecord.length > merkleTC.history.maxHistoryNodeNum) {
                merkleTC.history.historyRecord = merkleTC.history.historyRecord.slice(merkleTC.history.historyRecord.length - merkleTC.history.maxHistoryNodeNum);
                console.log(merkleTC.history.historyRecord);
            }
            merkleTC.renderChart(data,merkleTC.applicationDiv.id);
        }

    });
    actionButtonGroupDiv.append(showFocusNodeTreeButton);

    var actionDiv = document.createElement("div");
    actionDiv.setAttribute("id", "action-box");
    actionDiv.append(focusNodeDiv);
    actionDiv.append(actionButtonGroupDiv);
    document.getElementById(div_id).append(actionDiv);


    resetButton.addEventListener("click", function () {
        document.getElementById("chain-svg-div").innerHTML = "";
        merkleTC.history.historyRecord = [merkleTC.svg.treeData];

        var historyBar = document.getElementById("history-bar-div");
        historyBar.innerHTML = "";
        var historyRec = Object.assign([], merkleTC.history.historyRecord)
        merkleTC.generateNewHistoryBar(historyBar, historyRec);

        var chartSVG = merkleTC.updateSvg(merkleTC.svg.treeData);
        document.getElementById("chain-svg-div").append(chartSVG.svgElement.node());
        merkleTC.showChildNodeTree()
    });
    merkleTC.showChildNodeTree();
}


merkleTC.generateFocusNodeDiv = function (focusNode) {
    var html = '<div id="focus-node-img"><img src="' + focusNode.img + '"></div><div id="focus-node-nickname">' + focusNode.nickname + '</div>';
    var showFocusNodeButton = document.getElementById("show-focus-node-tree-button");
    showFocusNodeButton.setAttribute("focus-node-id", focusNode.customerId);
    showFocusNodeButton.setAttribute("data", JSON.stringify(focusNode.data));
    return html;
}

merkleTC.generateNewHistoryBar = function (historyBar, historyRec) {
    for (key in historyRec) {
        var aLink = document.createElement("a");
        aLink.setAttribute("history-seq", key);
        aLink.setAttribute("class", "history-click-node");
        aLink.addEventListener("click", function () {
            var index = this.getAttribute("history-seq");
            console.log(index);
            var data = merkleTC.history.historyRecord[index];
            merkleTC.history.historyRecord = merkleTC.history.historyRecord.slice(0, parseInt(index) + 1);
            console.log(merkleTC.history);
            console.log(data);
            merkleTC.renderChart(data,merkleTC.applicationDiv.id);
        });
        var image = document.createElement("img");
        image.setAttribute("src", historyRec[key].img == "" ? merkleTC.svg.personIconBase64 : historyRec[key].img);

        var arrow = document.createElement("div");
        arrow.setAttribute("class", 'history-path');
        arrow.innerText = ">";

        aLink.append(image);
        historyBar.append(aLink);
        historyBar.append(arrow);
        // console.log(aLink);
        // console.log(historyRec[key].img);
    }
}

merkleTC.showChildNodeTree = function () {
    d3.selectAll("circle").on("click", function (d) {
        var focusNodeCustomerId = this.getAttribute("id").replace("-circle", "");
        merkleTC.getNickName(focusNodeCustomerId, function (data) {
            var textId = focusNodeCustomerId + '-nickname';
            var text = document.getElementById(textId);            
            text.innerHTML = data + "," + text.getAttribute("gender");
            var textDisplay = document.getElementById(textId).style.display;
            if (textDisplay == 'none' || textDisplay == '') {
                text.style.display = 'block';
            } else {
                text.style.display = 'none';
            }

            var img = document.getElementById(focusNodeCustomerId + "-img").children[0].getAttribute('href');
            var nickname = data;
            var focusNode = {
                customerId: focusNodeCustomerId,
                nickname: nickname,
                img: img,
                data: d.data
            }
            var focusNodeDiv = merkleTC.generateFocusNodeDiv(focusNode);
            document.getElementById("current-node-nickname").innerHTML = focusNodeDiv;
        });

        
    });
}

merkleTC.getNickName = function (customerId, callback) {
    var textId = customerId + '-nickname';
    var text = document.getElementById(textId);
    var encryptedNickname = text.getAttribute("encryptedNickname").trim();
    decryptNickename(encryptedNickname ,callback);
    
}

function decryptNickename (encryptedNickname, callback){
    d3.text("http://localhost/rsa-encryption/unencrypt/nickname/" + encryptedNickname).then(function (data, err) {
        callback(data);
    });
}
merkleTC.getRootTreeData = function (d) {
    var node = Object.assign({}, d)
    if (node.parent != null) {
        result = this.getRootTreeData(node.parent);
    } else {
        console.log(node.parent);
        console.log(node.data);
        result = node.data;
    }
    return result;
}

merkleTC.nodesFromTree = function (tree) {
    var nodes = [];
    var node = Object.assign({}, tree);
    var children = Object.assign([], node.children);
    delete node.children;
    // console.log(node);
    nodes.push(node);

    var chidrenNode = [];
    if (children.length > 0) {
        for (key in children) {
            chidrenNode = chidrenNode.concat(this.nodesFromTree(children[key]));
            // console.log(chidrenNode);
        }
    }
    return nodes.concat(chidrenNode);
}

merkleTC.treeToLink = function (tree) {
    var node = Object.assign({}, tree);
    var children = Object.assign([], node.children);
    var links = [];
    var link = {};
    if (children.length > 0) {
        for (key in children) {
            link.customerID = node.customerID;
            link.forwardUserID = children[key].customerID;
            link.value = node.value;
            link.pageURL = "";
            link.pageName = "";
            links.push(link);
            links = links.concat(this.treeToLink(children[key]));
        }
    }
    // console.log(links);
    return links;
}

function convertImgToBase64(url, callback, outputFormat) {
    var canvas = document.createElement('CANVAS'),
        ctx = canvas.getContext('2d'),
        img = new Image;
    img.crossOrigin = 'Anonymous';
    img.onload = function () {
        canvas.height = img.height;
        canvas.width = img.width;
        ctx.drawImage(img, 0, 0);
        var dataURL = canvas.toDataURL(outputFormat || 'image/png');
        callback.call(this, dataURL);
        canvas = null;
    };
    img.src = url;
}

//图片地址 
var imagePath = '/images/image.png';
convertImgToBase64(imagePath, function (base64Img) {
    // console.log(base64Img);
});